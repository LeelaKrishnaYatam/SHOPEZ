...
const NewProduct = () => {
  ...
  const handleNewProduct = async() =>{
    try{
      ...
      await axios.post('http://localhost:6001/add-new-product', {
        productName, productDescription, productMainImg, productCarousel: [productCarouselImg1, productCarouselImg2, productCarouselImg3], productSizes, productGender, productCategory, productNewCategory, productPrice, productDiscount
      }, {
  ...
  const handleImageUpload = async () => {
    ...
    const res = await axios.post('http://localhost:6001/upload-product-image', formData, {
      headers: { 'Content-Type': 'multipart/form-data', Authorization: `Bearer ${token}` }
    });
    setProductMainImg(res.data.imageUrl);
  };
